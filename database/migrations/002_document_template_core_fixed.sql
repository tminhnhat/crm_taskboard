...
-- Updated: September 2025 - Fixed table name to match frontend expectations
...
-- ===================================================================
-- 1. TEMPLATES TABLE - Core table for template management (renamed from document_templates)
-- ===================================================================
...
CREATE INDEX IF NOT EXISTS idx_templates_type ON dulieu_congviec.templates(template_type);
CREATE INDEX IF NOT EXISTS idx_templates_active ON dulieu_congviec.templates(is_active);
CREATE INDEX IF NOT EXISTS idx_templates_extension ON dulieu_congviec.templates(file_extension);
...
CREATE TABLE IF NOT EXISTS dulieu_congviec.documents (
    ...
    template_id INTEGER,                   -- REFERENCES templates(template_id) - which template was used
...
--     FOREIGN KEY (template_id) REFERENCES dulieu_congviec.templates(template_id) ON DELETE SET NULL;
...
DROP TRIGGER IF EXISTS update_templates_updated_at ON dulieu_congviec.templates;
CREATE TRIGGER update_templates_updated_at 
    BEFORE UPDATE ON dulieu_congviec.templates 
...
-- Templates policies
DROP POLICY IF EXISTS "Enable read access for all users" ON dulieu_congviec.templates;
CREATE POLICY "Enable read access for all users" ON dulieu_congviec.templates FOR SELECT USING (true);

DROP POLICY IF EXISTS "Enable insert for all users" ON dulieu_congviec.templates;
CREATE POLICY "Enable insert for all users" ON dulieu_congviec.templates FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Enable update for all users" ON dulieu_congviec.templates;
CREATE POLICY "Enable update for all users" ON dulieu_congviec.templates FOR UPDATE USING (true);

DROP POLICY IF EXISTS "Enable delete for all users" ON dulieu_congviec.templates;
CREATE POLICY "Enable delete for all users" ON dulieu_congviec.templates FOR DELETE USING (true);
...
INSERT INTO dulieu_congviec.templates (template_name, template_type, file_url, file_extension, description) VALUES
...
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'dulieu_congviec' AND table_name = 'templates') THEN
        RAISE NOTICE '✅ dulieu_congviec.templates table created successfully';
    END IF;
    ...
    RAISE NOTICE '🔧 FIXED: Table name changed from document_templates to templates to match frontend expectations';
END $$;

-- Query to show table structure
\d dulieu_congviec.templates;

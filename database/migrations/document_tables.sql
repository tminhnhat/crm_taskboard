...
CREATE TABLE IF NOT EXISTS dulieu_congviec.documents (
    ...
    customer_id INTEGER NOT NULL REFERENCES dulieu_congviec.customers(customer_id),
    collateral_id INTEGER REFERENCES dulieu_congviec.collaterals(collateral_id),
    assessment_id INTEGER REFERENCES dulieu_congviec.credit_assessments(assessment_id),
...
CREATE POLICY "Allow read access to active documents" ON dulieu_congviec.documents
    FOR SELECT
    USING (status = 'active');

CREATE POLICY "Allow insert for authenticated users" ON dulieu_congviec.documents
    FOR INSERT
    WITH CHECK (true);

CREATE POLICY "Allow update own documents" ON dulieu_congviec.documents
    FOR UPDATE
    USING (status = 'active')
    WITH CHECK (true);

CREATE POLICY "Allow soft delete own documents" ON dulieu_congviec.documents
    FOR DELETE
    USING (status = 'active');

-- Create policies for document_templates
CREATE POLICY "Allow read access to active templates" ON dulieu_congviec.document_templates
    FOR SELECT
    USING (status = 'active');

CREATE POLICY "Allow insert for authenticated users" ON dulieu_congviec.document_templates
    FOR INSERT
    WITH CHECK (true);

CREATE POLICY "Allow update own templates" ON dulieu_congviec.document_templates
    FOR UPDATE
    USING (status = 'active')
    WITH CHECK (true);

CREATE POLICY "Allow soft delete own templates" ON dulieu_congviec.document_templates
    FOR DELETE
    USING (status = 'active');
